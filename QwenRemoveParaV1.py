...
def process_single_image(image_path, remove_iterations=1, comfyui_folder=None, workflow_path=None):
    ...
    for iteration in range(remove_iterations):
        ...
        if output_file and os.path.exists(output_file):
            ...
            print(f"    已生成输出: {os.path.basename(output_file)}")
        else:
            print(f"    未找到迭代 {iteration+1} 的输出文件")
...
def main():
    """
    主函数：处理命令行参数中的图像文件
    用法: python QwenRemoveParaV1.py 图像1.jpg 图像2.png ...
    """
    # 解析命令行参数
    image_paths = []
    ...
    if len(sys.argv) < 2:
        print("使用方法:")
        print("  python QwenRemoveParaV1.py 图像1.jpg 图像2.png ...")
    ...
    for i, image_path in enumerate(image_paths):
        ...
        print(f"    完成，共生成 {len(output_files)} 张处理结果")
