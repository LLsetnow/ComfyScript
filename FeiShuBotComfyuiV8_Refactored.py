...
class FeishuAPI:
    ...
    def get_tenant_access_token() -> Optional[str]:
        ...
        token_url = f"{AppConfig.FEISHU_API_BASE}/auth/v3/tenant_access_token/internal"
    ...
    @staticmethod
    def download_image(image_key: str, message_id: str) -> Optional[str]:
        """从飞书下载图片并返回保存路径"""
        try:
            import requests
        except ImportError:
        ...
        token = FeishuAPI.get_tenant_access_token()
        if not token:
            return None

        resource_url = f"{AppConfig.FEISHU_API_BASE}/im/v1/messages/{message_id}/resources/{image_key}?type=image"
        ...
        try:
            ...
            temp_path = os.path.join(AppConfig.COMFYUI_INPUT_FOLDER, temp_filename)
    ...
    def upload_image(image_path: str) -> Optional[str]:
        ...
        try:
            ...
            print("  [ERROR] requests或requests_toolbelt库未安装")
            return None
        ...
        upload_url = f"{AppConfig.FEISHU_API_BASE}/im/v1/images"
    ...
    def upload_and_send_image(self, chat_id: str, image_path: str,
        ...
        try:
            ...
            if caption:
                ...
                self.send_message(chat_id, caption_content, "text")

            # 上传并发送图片
            image_key = FeishuAPI.upload_image(image_path)
            if image_key:
