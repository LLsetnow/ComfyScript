...
class UserDatabase:
    ...
    def clear_all_task_numbers(self):
        """清空所有用户的任务序号"""
        with self.lock:
            for user_id in self.data["users"]:
                self.data["users"][user_id]["task_numbers"] = []
            self.save()
...
AUTHORIZED_USER_IDS = CONFIG["telegram"]["authorized_user_ids"]  # 设置授权用户的Telegram ID列表, 为空则允许所有用户
            ...
            for update in updates["result"]:
                ...
                # 处理文本消息
                    ...
                    if text == "/start":
                        ...
                        # 添加到用户任务列表
                        try:
                            db.add_task_number(edit_state["user_id"], current_task_number)
                            print(f"任务已添加到用户列表")
                        except Exception as e:
                            print(f"添加任务序号失败: {e}")
                            send_message(chat_id, "❌ 任务添加失败")
                            del edit_prompt_states[chat_id]
                            continue
